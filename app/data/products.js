export const products = [
    {
        id: "1",
        slug: "aile-evi-classic",
        images: ['/slider.jpg', '/slider2.jpg', '/slider.jpg', '/slider2.jpg'],
        size: "120m²",
        rooms: 4,
        price: {
            az: "85,000 AZN",
            ru: "85,000 AZN"
        },
        name: {
            az: "Ailə Evi Classic",
            ru: "Семейный Дом Classic"
        },
        description: {
            az: "2 mərtəbəli, 4 otaqlı müasir taxta ev",
            ru: "2-этажный, 4-комнатный современный деревянный дом"
        },
        features: {
            az: [
                "2 mərtəbəli",
                "4 yataq otağı",
                "2 hamam otağı",
                "Geniş mətbəx",
                "Qaraj",
                "Bağça",
                "İstilik sistemi",
                "Su təchizatı",
                "Elektrik sistemi"
            ],
            ru: [
                "2 этажа",
                "4 спальни",
                "2 ванные комнаты",
                "Просторная кухня",
                "Гараж",
                "Сад",
                "Система отопления",
                "Водоснабжение",
                "Электрическая система"
            ]
        },
        specifications: {
            construction: {
                az: "Yüksək keyfiyyətli taxta",
                ru: "Высококачественная древесина"
            },
            warranty: {
                az: "10 il zəmanət",
                ru: "10 лет гарантии"
            },
            buildTime: {
                az: "3 ay",
                ru: "3 месяца"
            }
        }
    },
    {
        id: "2",
        slug: "kompakt-aile-evi",
        images: ['/slider2.jpg', '/slider.jpg', '/slider2.jpg', '/slider.jpg'],
        size: "85m²",
        rooms: 3,
        price: {
            az: "65,000 AZN",
            ru: "65,000 AZN"
        },
        name: {
            az: "Kompakt Ailə Evi",
            ru: "Компактный Семейный Дом"
        },
        description: {
            az: "3 otaqlı kompakt və rahat taxta ev",
            ru: "3-комнатный компактный и уютный деревянный дом"
        },
        features: {
            az: [
                "1 mərtəbəli",
                "3 yataq otağı",
                "1 hamam otağı",
                "Mətbəx",
                "Terras",
                "İstilik sistemi",
                "Su təchizatı",
                "Elektrik sistemi"
            ],
            ru: [
                "1 этаж",
                "3 спальни",
                "1 ванная комната",
                "Кухня",
                "Терраса",
                "Система отопления",
                "Водоснабжение",
                "Электрическая система"
            ]
        },
        specifications: {
            construction: {
                az: "Yüksək keyfiyyətli taxta",
                ru: "Высококачественная древесина"
            },
            warranty: {
                az: "10 il zəmanət",
                ru: "10 лет гарантии"
            },
            buildTime: {
                az: "2 ay",
                ru: "2 месяца"
            }
        }
    },
    {
        id: "3",
        slug: "premium-villa",
        images: ['/slider.jpg', '/slider2.jpg', '/slider.jpg', '/slider2.jpg'],
        size: "150m²",
        rooms: 5,
        price: {
            az: "95,000 AZN",
            ru: "95,000 AZN"
        },
        name: {
            az: "Premium Villa",
            ru: "Премиум Вилла"
        },
        description: {
            az: "5 otaqlı lüks taxta villa",
            ru: "5-комнатная люкс деревянная вилла"
        },
        features: {
            az: [
                "2 mərtəbəli",
                "5 yataq otağı",
                "3 hamam otağı",
                "Geniş mətbəx",
                "2 Qaraj",
                "Böyük bağça",
                "İstilik sistemi",
                "Su təchizatı",
                "Elektrik sistemi"
            ],
            ru: [
                "2 этажа",
                "5 спален",
                "3 ванные комнаты",
                "Просторная кухня",
                "2 Гаража",
                "Большой сад",
                "Система отопления",
                "Водоснабжение",
                "Электрическая система"
            ]
        },
        specifications: {
            construction: {
                az: "Yüksək keyfiyyətli taxta",
                ru: "Высококачественная древесина"
            },
            warranty: {
                az: "10 il zəmanət",
                ru: "10 лет гарантии"
            },
            buildTime: {
                az: "4 ay",
                ru: "4 месяца"
            }
        }
    },
    {
        id: "4",
        slug: "modern-aile-evi",
        images: ['/slider2.jpg', '/slider.jpg', '/slider2.jpg', '/slider.jpg'],
        size: "110m²",
        rooms: 4,
        price: {
            az: "78,000 AZN",
            ru: "78,000 AZN"
        },
        name: {
            az: "Modern Ailə Evi",
            ru: "Современный Семейный Дом"
        },
        description: {
            az: "4 otaqlı müasir dizaynlı ev",
            ru: "4-комнатный дом с современным дизайном"
        },
        features: {
            az: [
                "2 mərtəbəli",
                "4 yataq otağı",
                "2 hamam otağı",
                "Müasir mətbəx",
                "Qaraj",
                "Terras",
                "İstilik sistemi",
                "Su təchizatı",
                "Smart ev sistemi"
            ],
            ru: [
                "2 этажа",
                "4 спальни",
                "2 ванные комнаты",
                "Современная кухня",
                "Гараж",
                "Терраса",
                "Система отопления",
                "Водоснабжение",
                "Система умного дома"
            ]
        },
        specifications: {
            construction: {
                az: "Yüksək keyfiyyətli taxta",
                ru: "Высококачественная древесина"
            },
            warranty: {
                az: "10 il zəmanət",
                ru: "10 лет гарантии"
            },
            buildTime: {
                az: "3 ay",
                ru: "3 месяца"
            }
        }
    },
    {
        id: "5",
        slug: "eco-yasayis-evi",
        images: ['/slider.jpg', '/slider2.jpg', '/slider.jpg', '/slider2.jpg'],
        size: "95m²",
        rooms: 3,
        price: {
            az: "72,000 AZN",
            ru: "72,000 AZN"
        },
        name: {
            az: "Eco Yaşayış Evi",
            ru: "Эко Жилой Дом"
        },
        description: {
            az: "Ekoloji təmiz materiallardan hazırlanmış ev",
            ru: "Дом из экологически чистых материалов"
        },
        features: {
            az: [
                "1 mərtəbəli",
                "3 yataq otağı",
                "2 hamam otağı",
                "Mətbəx",
                "Günəş panelləri",
                "Bio bağça",
                "Təbii istilik sistemi",
                "Yağış suyu yığma sistemi",
                "Eco materiallar"
            ],
            ru: [
                "1 этаж",
                "3 спальни",
                "2 ванные комнаты",
                "Кухня",
                "Солнечные панели",
                "Био сад",
                "Натуральная система отопления",
                "Система сбора дождевой воды",
                "Эко материалы"
            ]
        },
        specifications: {
            construction: {
                az: "Eco taxta",
                ru: "Эко древесина"
            },
            warranty: {
                az: "15 il zəmanət",
                ru: "15 лет гарантии"
            },
            buildTime: {
                az: "2.5 ay",
                ru: "2.5 месяца"
            }
        }
    },
    {
        id: "6",
        slug: "smart-villa",
        images: ['/slider2.jpg', '/slider.jpg', '/slider2.jpg', '/slider.jpg'],
        size: "160m²",
        rooms: 5,
        price: {
            az: "135,000 AZN",
            ru: "135,000 AZN"
        },
        name: {
            az: "Smart Villa",
            ru: "Смарт Вилла"
        },
        description: {
            az: "Tam avtomatlaşdırılmış ağıllı ev sistemi",
            ru: "Полностью автоматизированная система умного дома"
        },
        features: {
            az: [
                "2 mərtəbəli",
                "5 yataq otağı",
                "3 hamam otağı",
                "Smart mətbəx",
                "2 Qaraj",
                "Smart bağça",
                "Avtomatik istilik",
                "Təhlükəsizlik sistemi",
                "Səs idarəetmə"
            ],
            ru: [
                "2 этажа",
                "5 спален",
                "3 ванные комнаты",
                "Смарт кухня",
                "2 Гаража",
                "Смарт сад",
                "Автоматическое отопление",
                "Система безопасности",
                "Голосовое управление"
            ]
        },
        specifications: {
            construction: {
                az: "Premium taxta",
                ru: "Премиум древесина"
            },
            warranty: {
                az: "20 il zəmanət",
                ru: "20 лет гарантии"
            },
            buildTime: {
                az: "4 ay",
                ru: "4 месяца"
            }
        }
    },
    {
        id: "7",
        slug: "yay-evi",
        images: ['/slider.jpg', '/slider2.jpg', '/slider.jpg', '/slider2.jpg'],
        size: "70m²",
        rooms: 2,
        price: {
            az: "45,000 AZN",
            ru: "45,000 AZN"
        },
        name: {
            az: "Yay Evi",
            ru: "Летний Дом"
        },
        description: {
            az: "İstirahət üçün ideal yay evi",
            ru: "Идеальный летний дом для отдыха"
        },
        features: {
            az: [
                "1 mərtəbəli",
                "2 yataq otağı",
                "1 hamam otağı",
                "Açıq mətbəx",
                "Böyük terras",
                "Barbekyü sahəsi",
                "Hovuz",
                "Bağça",
                "İstirahət zonası"
            ],
            ru: [
                "1 этаж",
                "2 спальни",
                "1 ванная комната",
                "Открытая кухня",
                "Большая терраса",
                "Зона барбекю",
                "Бассейн",
                "Сад",
                "Зона отдыха"
            ]
        },
        specifications: {
            construction: {
                az: "Yüngül taxta",
                ru: "Легкая древесина"
            },
            warranty: {
                az: "5 il zəmanət",
                ru: "5 лет гарантии"
            },
            buildTime: {
                az: "1.5 ay",
                ru: "1.5 месяца"
            }
        }
    },
    {
        id: "8",
        slug: "dag-evi",
        images: ['/slider2.jpg', '/slider.jpg', '/slider2.jpg', '/slider.jpg'],
        size: "90m²",
        rooms: 3,
        price: {
            az: "68,000 AZN",
            ru: "68,000 AZN"
        },
        name: {
            az: "Dağ Evi",
            ru: "Горный Дом"
        },
        description: {
            az: "Dağ mənzərəli müasir ev",
            ru: "Современный дом с горным видом"
        },
        features: {
            az: [
                "2 mərtəbəli",
                "3 yataq otağı",
                "2 hamam otağı",
                "Panoramik mətbəx",
                "Şöminə",
                "Qaraj",
                "Terras",
                "İstilik sistemi",
                "Mənzərəli eyvan"
            ],
            ru: [
                "2 этажа",
                "3 спальни",
                "2 ванные комнаты",
                "Панорамная кухня",
                "Камин",
                "Гараж",
                "Терраса",
                "Система отопления",
                "Панорамный балкон"
            ]
        },
        specifications: {
            construction: {
                az: "Davamlı taxta",
                ru: "Прочная древесина"
            },
            warranty: {
                az: "12 il zəmanət",
                ru: "12 лет гарантии"
            },
            buildTime: {
                az: "3 ay",
                ru: "3 месяца"
            }
        }
    },
    {
        id: "9",
        slug: "sahil-villasi",
        images: ['/slider.jpg', '/slider2.jpg', '/slider.jpg', '/slider2.jpg'],
        size: "140m²",
        rooms: 4,
        price: {
            az: "115,000 AZN",
            ru: "115,000 AZN"
        },
        name: {
            az: "Sahil Villası",
            ru: "Прибрежная Вилла"
        },
        description: {
            az: "Dəniz kənarı lüks villa",
            ru: "Люкс вилла у моря"
        },
        features: {
            az: [
                "2 mərtəbəli",
                "4 yataq otağı",
                "3 hamam otağı",
                "Dəniz mənzərəli mətbəx",
                "Hovuz",
                "Qaraj",
                "Geniş terras",
                "Bağça",
                "Çimərlik girişi"
            ],
            ru: [
                "2 этажа",
                "4 спальни",
                "3 ванные комнаты",
                "Кухня с видом на море",
                "Бассейн",
                "Гараж",
                "Просторная терраса",
                "Сад",
                "Выход к пляжу"
            ]
        },
        specifications: {
            construction: {
                az: "Premium taxta",
                ru: "Премиум древесина"
            },
            warranty: {
                az: "15 il zəmanət",
                ru: "15 лет гарантии"
            },
            buildTime: {
                az: "4 ay",
                ru: "4 месяца"
            }
        }
    },
    {
        id: "10",
        slug: "bag-evi",
        images: ['/slider2.jpg', '/slider.jpg', '/slider2.jpg', '/slider.jpg'],
        size: "80m²",
        rooms: 2,
        price: {
            az: "55,000 AZN",
            ru: "55,000 AZN"
        },
        name: {
            az: "Bağ Evi",
            ru: "Садовый Дом"
        },
        description: {
            az: "Geniş bağçalı ailə evi",
            ru: "Семейный дом с большим садом"
        },
        features: {
            az: [
                "1 mərtəbəli",
                "2 yataq otağı",
                "1 hamam otağı",
                "Mətbəx",
                "Terras",
                "Böyük bağça",
                "İstilik sistemi",
                "Meyvə bağı",
                "İstirahət zonası"
            ],
            ru: [
                "1 этаж",
                "2 спальни",
                "1 ванная комната",
                "Кухня",
                "Терраса",
                "Большой сад",
                "Система отопления",
                "Фруктовый сад",
                "Зона отдыха"
            ]
        },
        specifications: {
            construction: {
                az: "Təbii taxta",
                ru: "Натуральная древесина"
            },
            warranty: {
                az: "8 il zəmanət",
                ru: "8 лет гарантии"
            },
            buildTime: {
                az: "2 ay",
                ru: "2 месяца"
            }
        }
    },
    {
        id: "11",
        slug: "qonak-evi",
        images: ['/slider.jpg', '/slider2.jpg', '/slider.jpg', '/slider2.jpg'],
        size: "100m²",
        rooms: 3,
        price: {
            az: "75,000 AZN",
            ru: "75,000 AZN"
        },
        name: {
            az: "Qonaq Evi",
            ru: "Гостевой Дом"
        },
        description: {
            az: "Qonaqlar üçün rahat ev",
            ru: "Уютный дом для гостей"
        },
        features: {
            az: [
                "2 mərtəbəli",
                "3 yataq otağı",
                "2 hamam otağı",
                "Geniş qonaq otağı",
                "Mətbəx",
                "Terras",
                "İstilik sistemi",
                "Bağça",
                "Avtodayanacaq"
            ],
            ru: [
                "2 этажа",
                "3 спальни",
                "2 ванные комнаты",
                "Просторная гостиная",
                "Кухня",
                "Терраса",
                "Система отопления",
                "Сад",
                "Парковка"
            ]
        },
        specifications: {
            construction: {
                az: "Yüksək keyfiyyətli taxta",
                ru: "Высококачественная древесина"
            },
            warranty: {
                az: "10 il zəmanət",
                ru: "10 лет гарантии"
            },
            buildTime: {
                az: "3 ay",
                ru: "3 месяца"
            }
        }
    },
    {
        id: "12",
        slug: "luks-kotec",
        images: ['/slider2.jpg', '/slider.jpg', '/slider2.jpg', '/slider.jpg'],
        size: "180m²",
        rooms: 6,
        price: {
            az: "145,000 AZN",
            ru: "145,000 AZN"
        },
        name: {
            az: "Lüks Kottec",
            ru: "Люкс Коттедж"
        },
        description: {
            az: "Premium sinif lüks kottec",
            ru: "Премиум класс люкс коттедж"
        },
        features: {
            az: [
                "2 mərtəbəli",
                "6 yataq otağı",
                "4 hamam otağı",
                "Geniş mətbəx",
                "2 Qaraj",
                "Hovuz",
                "Spa zona",
                "Smart sistem",
                "Fitness zalı"
            ],
            ru: [
                "2 этажа",
                "6 спален",
                "4 ванные комнаты",
                "Просторная кухня",
                "2 Гаража",
                "Бассейн",
                "Спа зона",
                "Смарт система",
                "Фитнес зал"
            ]
        },
        specifications: {
            construction: {
                az: "Lüks taxta",
                ru: "Люкс древесина"
            },
            warranty: {
                az: "25 il zəmanət",
                ru: "25 лет гарантии"
            },
            buildTime: {
                az: "5 ay",
                ru: "5 месяца"
            }
        }
    }
];

export const translations = {
    az: {
        specifications: "Spesifikasiyalar",
        features: "Xüsusiyyətlər",
        construction: "İnşaat Materialı:",
        warranty: "Zəmanət:",
        buildTime: "Tikilmə Müddəti:",
        contact: "WhatsApp ilə əlaqə",
        size: "Sahə:",
        rooms: "Otaq sayı:",
        gallery: "Qalereya",
        title: "Layihələr",
        subtitle: "Təbiətlə harmoniyada yaşamaq üçün ən yaxşı seçim",
        viewAll: "Bütün Layihələr",
        viewDetails: "Ətraflı Bax",
        bestSellersTitle: "Layihələrimiz",
        bestSellersSubtitle: "",
        all: "Bütün Proyektlər",
        smallHouses: "Kiçik Evlər (<100m²)",
        mediumHouses: "Orta Evlər (100-150m²)",
        largeHouses: "Böyük Evlər (>150m²)",
        sortBy: "Sıralama",
        priceAsc: "Qiymət (Artan)",
        priceDesc: "Qiymət (Azalan)",
        sizeAsc: "Sahə (Artan)",
        sizeDesc: "Sahə (Azalan)",
    },
    ru: {
        specifications: "Спецификации",
        features: "Характеристики",
        construction: "Строительный материал:",
        warranty: "Гарантия:",
        buildTime: "Срок строительства:",
        contact: "Связаться через WhatsApp",
        size: "Площадь:",
        rooms: "Количество комнат:",
        gallery: "Галерея",
        title: "Проекты",
        subtitle: "Лучший выбор для жизни в гармонии с природой",
        viewAll: "Все Проекты",
        viewDetails: "Подробнее",
        bestSellersTitle: "Наши  проекты",
        bestSellersSubtitle: "",
        all: "Все Проекты",
        smallHouses: "Малые Дома (<100m²)",
        mediumHouses: "Средние Дома (100-150m²)",
        largeHouses: "Большие Дома (>150m²)",
        sortBy: "Сортировка",
        priceAsc: "Цена (По возрастанию)",
        priceDesc: "Цена (По убыванию)",
        sizeAsc: "Площадь (По возрастанию)",
        sizeDesc: "Площадь (По убыванию)",
    }
};

// Ürünleri API'den almak için fonksiyon
export async function getProducts(locale = 'az') {
    try {
        const response = await fetch(`/api/home/products?locale=${locale}`);
        if (!response.ok) {
            throw new Error('Ürünler yüklenemedi');
        }
        const data = await response.json();
        return data;
    } catch (error) {
        console.error('Ürünleri getirme hatası:', error);
        return [];
    }
}

// Tek bir ürün getirmek için fonksiyon
export async function getProductBySlug(slug, locale = 'az') {
    try {
        const products = await getProducts(locale);
        return products.find(product => product.slug === slug);
    } catch (error) {
        console.error('Ürün getirme hatası:', error);
        return null;
    }
}

// Varsayılan ürünler (fallback için)
export const defaultProducts = [
    {
        id: "1",
        slug: "aile-evi-classic",
        images: ['/slider.jpg', '/slider2.jpg', '/slider.jpg', '/slider2.jpg'],
        size: "120m²",
        rooms: 4,
        price: {
            az: "85,000 AZN",
            ru: "85,000 AZN"
        },
        name: {
            az: "Ailə Evi Classic",
            ru: "Семейный Дом Classic"
        },
        description: {
            az: "2 mərtəbəli, 4 otaqlı müasir taxta ev",
            ru: "2-этажный, 4-комнатный современный деревянный дом"
        },
        features: {
            az: [
                "2 mərtəbəli",
                "4 yataq otağı",
                "2 hamam otağı",
                "Geniş mətbəx",
                "Qaraj",
                "Bağça",
                "İstilik sistemi",
                "Su təchizatı",
                "Elektrik sistemi"
            ],
            ru: [
                "2 этажа",
                "4 спальни",
                "2 ванные комнаты",
                "Просторная кухня",
                "Гараж",
                "Сад",
                "Система отопления",
                "Водоснабжение",
                "Электрическая система"
            ]
        },
        specifications: {
            construction: {
                az: "Yüksək keyfiyyətli taxta",
                ru: "Высококачественная древесина"
            },
            warranty: {
                az: "10 il zəmanət",
                ru: "10 лет гарантии"
            },
            buildTime: {
                az: "3 ay",
                ru: "3 месяца"
            }
        }
    }
    // Diğer varsayılan ürünler kaldırıldı
]; 